    ...
    <section className={`${styles.flexCenter} ${styles.paddingY} flex-col`}>
      <div className={`${styles.flexStart} md:flex-row flex-col mb-8 w-full`}>
        <div className="flex-1 flex-col justify-start mr-10">
          <img src={logo} alt="hoobank" className="w-[266px] h-[62px] object-contain" />
            ...
            <div key={footerLink.key} className="flex flex-col ss:my-0 my-4 min-w-[150px]">
              ...
              <ul className="list-none mt-4">
                {footerLink.links.map((link,index)=>(
                  <li className={`font-poppins font-normal text-dimWhite hover:text-secondary cursor-pointer text-[16px] leading-[24px] ${
      ...
      <div className={`w-full flex justify-between md:flex-row flex-col items-center pt-6 border-t-[1px] border-t-[#3F3E45]`}>
        ...
        <div className="flex flex-row md:mt-0 mt-6">
          {socialMedia.map((social,index)=>(
            <img src={social.icon} alt={social.id} key={social.id} className={`w-[21px] h-[21px] object-contain cursor-pointer ${index!==socialMedia.length-1?'mr-4':'mr-0'}`} />
          ))}
        </div>
      </div>
    </section>
